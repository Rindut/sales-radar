...
func LeadFromStore(l store.Lead) Lead {
	original := domain.PrimaryDiscoverySourceNameFromTrace(l.SourceTrace, domain.Source(l.Source))
	enrichment := domain.EnrichmentSourceNamesFromTrace(l.SourceTrace, l.WebsiteEnrichmentStatus != "")
	priorityLevel := "Low"
	switch {
	case l.PriorityScore >= 80:
		priorityLevel = "High"
	case l.PriorityScore >= 50:
	...
	return Lead{
		...
		OriginalDiscoverySource:       original,
		EnrichmentSources:             append([]string(nil), enrichment...),
	}
}
